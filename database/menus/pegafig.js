...
async function buscaFigurinhas(bluxmd, from, info, body, command, NomeDoBot, NickDono) {
    try {
        ...
        if (!gifResults || gifResults.length === 0) {
            await bluxmd.sendMessage(from, { text: `❌ Nenhuma figurinha encontrada para: ${searchTerm}` }, { quoted: info });
            return;
        }
        ...
        if (enviados > 0) {
            ...
            await bluxmd.sendMessage(from, { text: `❌ Não consegui converter as figurinhas.` }, { quoted: info });
        }

    } catch (e) {
        console.log("Erro no comando buscafig:", e);
        await bluxmd.sendMessage(from, { text: "❌ Erro ao processar sua solicitação." }, { quoted: info });
    }
}
